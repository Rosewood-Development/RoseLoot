...
public final class SettingKey implements SettingHolder {
    ...
    public static final RoseSetting<Boolean> CALL_POSTLOOTGENERATEEVENT = create(RoseSetting.ofBoolean("call-postlootgenerateevent", false, "Should the PostLootGenerateEvent and PostLootTableGenerateEvent events be called after loot tables are run?", "You may need to enable this if you're using a plugin that uses the RoseLoot API"));
    ...
    public static final RoseSetting<Boolean> MODIFY_ENTITYDEATHEVENT_DROPS = create(RoseSetting.ofBoolean("modify-entitydeathevent-drops", true, "Should items and experience be added to the EntityDeathEvent drops?", "If false, items and experience generated by RoseLoot will be dropped on the ground directly"));
    public static final RoseSetting<Boolean> HEADS_USE_SHOPGUIPLUS_UUID = create(RoseSetting.ofBoolean("heads-use-shopguiplus-uuid", false, "Uses the same head UUID as ShopGUI+ which should allow selling the heads in the shop"));
