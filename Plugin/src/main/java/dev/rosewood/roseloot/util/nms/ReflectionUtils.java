    ...
    public static Method getMethodByName(Class<?> clazz, String name, Class<?>... parameterTypes) {
        try {
            ...
            try {
                ...
                throw new IllegalStateException("Failed to get method reflectively: [" + clazz.getName() + ";" + name + ";" + Arrays.stream(parameterTypes).map(Class::getName).collect(Collectors.joining(";")) + "]");
            }
        }
    }
